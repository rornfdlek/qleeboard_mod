<template xmlns:v-slot="http://www.w3.org/1999/XSL/Transform">
  <v-app>
    ...
    >
      ...
      <v-spacer></v-spacer>
      <div
        v-if="postData.user_srl === $store.getters.getUserSrl"
            ...
            <v-list dense v-if="comments.data.length > 0">
              <template v-for="(comment, index) in comments.data">
                ...
                >
                  ...
                  <v-list-tile-action
                    v-if="comment.user_srl === $store.getters.getUserSrl"
                  >
                    <v-btn
                      icon
...
export default {
  ...
  methods: {
    ...
    async getBoardName () {
      const res = await this.$http.get('/api/board/' + this.$route.params.boardId)
      this.boardName = res.data.result.name
    },
    async deletePost () {
      const res = await this.$http.delete('/api/board/post/' + this.$route.params.postId)
      this.$router.go(-1)
    },
    async deleteComment (commentId) {
      const res = await this.$http.delete('/api/board/comment/' + commentId)
      const commentRes = await this.getCommentData()
      this.setCommentData(commentRes)
    },
